    ...
    async def migration(self):
        ...
        for session_dict in sessions_to_migrate:
            try:
                ...
                if session_dict["chat_channel"]:
                    try:
                        await session_dict["chat_channel"].delete()
                    except Exception as e:
                        self.log.exception(f"Failed to delete channel", e)
                if session_dict["start_channel"]:
                    try:
                        await session_dict["start_channel"].delete()
                    except Exception as e:
                        self.log.exception(f"Failed to delete channel", e)
                if session_dict["config_channel"]:
                    try:
                ...
                if session_dict["info_channel"]:
                    try:
                        await session_dict["info_channel"].edit(name="📋dashboard")
                    except Exception as e:
                        self.log.exception(f"Failed to rename channel", e)
                else:
                    info_ow = {
